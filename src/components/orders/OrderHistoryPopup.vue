  ...
  <v-card title="Storico Ordine">
    ...
    >
      <template #[`item.status`]="{ item }">
        ...
        <div
          v-else-if="item.hasOwnProperty('confirmed')"
          class="d-flex align-center"
        >
          <div :style="{width: '15px', height: '15px', borderRadius: '50%', marginRight: '8px' ,backgroundColor: item.confirmed ? 'green' : 'red'}" />
          Confermato
        </div>
      </template>
