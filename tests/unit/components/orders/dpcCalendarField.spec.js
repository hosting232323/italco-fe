...
describe('DpcCalendarField', () => {
  ...
  it('mostra una sola chip per fascia, senza distinguere i veicoli per CAP', () => {
    const iso = todayIso();
    const wrapper = mountField({ slots: { [iso]: [{ start: '09:00', end: '11:30', caps: ['70056', '76011'] }] } });

    expect(wrapper.findAllComponents({ name: 'VChip' })).toHaveLength(1);
    expect(wrapper.text()).toContain('09:00-11:30');
    expect(wrapper.text()).not.toContain('70056');
  });

  it('evidenzia la fascia il cui orario coincide con quello selezionato', async () => {
    const iso = todayIso();
    const wrapper = mountField({
      slots: { [iso]: [{ start: '08:00', end: '12:00' }, { start: '14:00', end: '18:00' }] }
    });

    await wrapper.findAllComponents({ name: 'VChip' })[1].trigger('click');
    await wrapper.setProps({ slotStart: '14:00', slotEnd: '18:00' });
